...
namespace Laboratory.Chimera.Social
{
    ...
    public partial class CreatureBondingSystem : SystemBase
    {
        ...
        protected override void OnCreate()
        {
            ...
            if (_config == null)
            {
                Debug.LogError("CreatureBondingConfig not found in Resources/Configs/");
                return;
            }
        ...
        {
            Entities.WithAll<CreatureBondData, GeneticProfile>().ForEach((Entity entity, ref CreatureBondData bondData, ref GeneticProfile genetics) =>
            {
        ...
        private void CheckForBondingMoments(Entity entity, ref CreatureBondData bondData, GeneticProfile genetics)
        {
        ...
        private void CheckLifeMilestones(Entity entity, ref CreatureBondData bondData, GeneticProfile genetics)
        {
        ...
        private void CheckSharedDiscoveries(Entity entity, ref CreatureBondData bondData, GeneticProfile genetics)
        {
        ...
        private void CheckForNewLegacyConnections(Entity playerEntity, ref PlayerBondingHistory history)
        {
            ...
            Entities.WithAll<CreatureBondData, GeneticProfile>().ForEach((Entity creatureEntity, ref CreatureBondData bondData, ref GeneticProfile genetics) =>
            {
        ...
        {
            Entities.WithAll<CreatureBondData>().ForEach((Entity entity, ref CreatureBondData bondData) =>
            {
        ...
        private float CalculateMomentIntensity(BondingMomentType type, CreatureBondData bondData, GeneticProfile genetics)
        {
        ...
        private string GenerateMomentDescription(BondingMomentType type, GeneticProfile genetics)
        {
            return _config.GetMomentDescription(type, genetics.GetTraitNames().ToArray());
        }
        ...
        private float CalculateGeneticSimilarity(GeneticProfile current, GeneticProfile past)
        {
            if (current == null || past == null) return 0f;

            // Use the built-in genetic similarity calculation
            return current.GetGeneticSimilarity(past);
        }

        private float CalculateGeneticSimilarityFromHash(GeneticProfile current, uint geneticHash)
        {
        ...
        private string GenerateLegacyDescription(GeneticProfile current, PastBondData past, float similarity)
        {
        ...
        private float CalculateCreatureRarity(GeneticProfile genetics)
        {
            // Calculate rarity based on genetic traits
            float rarity = 0f;
            var traitNames = genetics.GetTraitNames();
            foreach (var trait in traitNames)
            {
                rarity += _config.GetTraitRarity(trait);
            }
            return traitNames.Count > 0 ? rarity / traitNames.Count : 0f;
        }
        ...
        private MemoryTriggerCondition[] GetMomentTriggerConditions(BondingMomentType type)
        {
            return _config.GetMomentTriggerConditions(type);
        }

        private MemoryTriggerCondition[] GetAncestralTriggerConditions(LegacyConnection connection)
        {
            return _config.GetAncestralTriggerConditions(connection.connectionType);
        }
        ...
        private bool CheckTimeCondition(MemoryTriggerCondition condition)
        {
