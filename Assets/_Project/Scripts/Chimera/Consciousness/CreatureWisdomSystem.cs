...
namespace Laboratory.Chimera.Consciousness
{
    ...
    public partial class CreatureWisdomSystem : SystemBase
    {
        ...
        {
            foreach (var (wisdomData, genetics, entity) in SystemAPI.Query<RefRW<WisdomData>, RefRO<VisualGeneticData>>().WithEntityAccess())
            {
                // Increase age
        ...
        private void AccumulateWisdomFromExperiences(Entity entity, ref WisdomData wisdomData, VisualGeneticData genetics, float deltaTime)
        {
        ...
        private void CheckWisdomMilestones(Entity entity, ref WisdomData wisdomData, VisualGeneticData genetics)
        {
            // Check for wisdom milestones
        ...
        private void UpdateSageStatus(Entity entity, ref WisdomData wisdomData, VisualGeneticData genetics)
        {
        ...
        private void ShareWisdomWithNearbyCreatures(Entity sageEntity, ref WisdomData sageWisdom)
        {
            ...
            {
                if (learningData.creatureId == sageWisdom.creatureId) return;

                // Check if wisdom sharing is beneficial
                if (CanBenefitFromWisdom(learningData, sageWisdom))
                {
                    var wisdomToShare = SelectWisdomToShare(sageWisdom, learningData);

                    if (wisdomToShare.HasValue)
                    {
                        TransferWisdom(sageEntity, ref sageWisdom, learnerEntity, ref learningData, wisdomToShare.Value);
                    }
                }
            }).Run();
        }
        ...
        private void TransferWisdom(Entity sageEntity, ref WisdomData sageWisdom, Entity learnerEntity, ref LearningData learningData, WisdomEntry wisdom)
        {
            ...
            var wisdomShared = new WisdomShared
            {
                ...
                lessonTitle = wisdom.description,
                timestamp = (float)SystemAPI.Time.ElapsedTime
            };
        ...
        private void ProcessLearningFromMentors(float deltaTime)
        {
            Entities.WithAll<LearningData>().WithoutBurst().ForEach((Entity entity, ref LearningData learningData) =>
            {
                ...
                {
                    ProcessLearningSession(entity, ref learningData);
                    learningData.learningTimer = 0f;
                }

                // Update learning capacity based on accumulated wisdom
                UpdateLearningCapacity(ref learningData);
            }).Run();
        }

        private void ProcessLearningSession(Entity entity, ref LearningData learningData)
        {
            // Gain wisdom from mentor relationship
            float learningGain = _config.baseLearningRate * learningData.learningMultiplier;
            learningData.accumulatedWisdom += learningGain;

            // Increase specific wisdom categories based on mentor's strengths
            if (_mentorships.ContainsKey(learningData.mentorId))
            {
                var mentorships = _mentorships[learningData.mentorId];
                var activeMentorship = mentorships.FirstOrDefault(m => m.studentId == learningData.creatureId);

                if (activeMentorship.mentorId != 0)
                {
                    // Apply mentor-specific learning benefits
                    ApplyMentorSpecificLearning(ref learningData, activeMentorship);
                }
            }

            // Check for learning milestones
            CheckLearningMilestones(entity, ref learningData);
        }
        ...
        {
            var wisdomData = GetComponent<WisdomData>(entity);
        ...
        private void GrantMilestoneAbilities(Entity entity, ref WisdomData wisdomData, WisdomMilestone milestone)
        {
            ...
            wisdomData.availableAbilities.AddRange(milestone.abilitiesGranted);
        }
        ...
        private void GrantSageAbilities(Entity entity, ref WisdomData wisdomData, SageLevel newLevel)
        {
            ...
            wisdomData.availableAbilities.AddRange(abilities);
        }
        ...
        private Entity SelectBestSage(Entity[] availableSages, GuidanceSeekingData seekingData)
        {
            ...
            {
                var wisdomData = GetComponent<WisdomData>(sage);
        ...
        {
            var sageWisdom = GetComponent<WisdomData>(sage);
