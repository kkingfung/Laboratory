...
namespace Laboratory.AI.ECS
{
    ...
    public unsafe class NativeArrayPool<T> : System.IDisposable where T : unmanaged
    {
        ...
        public void Clear()
        {
            ...
            // WARNING: Only dispose rented arrays if pool is being destroyed
            // This should only be called during pool disposal, not for regular cleanup
            if (_disposed)
            {
            ...
            {
                // Log warning if there are still rented arrays during non-disposal clear
                if (_rentedArrays.Count > 0)
                {
                    UnityEngine.Debug.LogWarning($"[NativeArrayPool] Clear() called with {_rentedArrays.Count} arrays still rented. " +
                                               "This may indicate a memory leak. Arrays will not be disposed to prevent crashes.");
                }
            }
