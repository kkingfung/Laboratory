...
namespace Laboratory.Core.Replay
{
    ...
    public class ReplayCameraFollow : MonoBehaviour
    {
        ...
        [SerializeField] private UnityEngine.MonoBehaviour _cinemachineBrain; // Temp fix: was CinemachineBrain
        [SerializeField] private UnityEngine.MonoBehaviour _followCamera; // Temp fix: was CinemachineVirtualCamera
        [SerializeField] private UnityEngine.MonoBehaviour _cinematicCamera; // Temp fix: was CinemachineVirtualCamera
        [SerializeField] private UnityEngine.MonoBehaviour _overviewCamera; // Temp fix: was CinemachineVirtualCamera
        ...
        private void SetupFollowCamera()
        {
            ...
            // Set the follow target using reflection to avoid compilation errors
            // This is a temporary fix until proper Cinemachine types are available
            var followProperty = _followCamera.GetType().GetProperty("Follow");
            if (followProperty != null)
                followProperty.SetValue(_followCamera, _targetActor);
            
            var lookAtProperty = _followCamera.GetType().GetProperty("LookAt");
            if (lookAtProperty != null)
                lookAtProperty.SetValue(_followCamera, _targetActor);
            
            // Configure follow camera settings using CinemachineVirtualCamera's properties
            // Note: In Cinemachine 2.x, component configuration is handled through the virtual camera
            // Additional configuration would be done through the camera's components (Composer, Transposer, etc.)
        }
        ...
        private void SetupCinematicCamera()
        {
            ...
            {
                // Set the follow target using reflection to avoid compilation errors
                var followProperty = _cinematicCamera.GetType().GetProperty("Follow");
                if (followProperty != null)
                    followProperty.SetValue(_cinematicCamera, _targetActor);
                
                var lookAtProperty = _cinematicCamera.GetType().GetProperty("LookAt");
                if (lookAtProperty != null)
                    lookAtProperty.SetValue(_cinematicCamera, _targetActor);
            }
            
            // Configure cinematic camera for wide shots
            // Note: In Cinemachine 2.x, component configuration is handled through the virtual camera
            // Additional configuration would be done through the camera's components for cinematic effects
        }
